...
def main():
    ...
    with ab1.container(border=True,height=620):
        plh1 = st.empty()
        with st.spinner('Cargando...'):
            img1 = asyncio.run(get_random_image('data-science'))
        if 'https' in img1:
    ...
    with ab2.container(border=True,height=620):
        plh2 = st.empty()
        with st.spinner('Cargando...'):
    ...
    with ab3.container(border=True,height=620):
        plh3 = st.empty()
        with st.spinner('Cargando...'):
...
with open('styles_blog.css') as f:
